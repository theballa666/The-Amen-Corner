...
writeNavLeft(true); //show navigation for comic pages. to toggle either images or text for nav, set this to true or false.
writeNavRight(true);
...
writePageDate(".writePageDate"); //writes the date of upload
...
function writePageDate(div) {
  if (pgData.length >= pg) {
    document.querySelector(div).innerHTML = `<p>Uploaded: ${pgData[pg - 1].date}</p>`;
  }
}
...
//function for the reacts feature under each comic
function writeReactions(div) {
  if (pgData.length >= pg) {
    document.querySelector(div).innerHTML = `<ws-widget type="reactions" name="${pgData[pg - 1].reactionName}" wid="11" auto></ws-widget>`;
  }
}

//function for the comments feature under each comic
function writeComments(div) {
  if (pgData.length >= pg) {
    document.querySelector(div).innerHTML = `<ws-widget type="comments" name="${pgData[pg - 1].commentName}" iid="13267"></ws-widget>`;
  }
}

function firstButton(imageToggle) {
    if (pg > 1) {
        return `<a href="?pg=` + 1 + navScrollTo + `"/>` + imgOrText(imageToggle, 0) + `</a>`;
    } else {
        return !imageToggle ? imgOrText(imageToggle, 0) : ``;
    }
}

function prevButton(imageToggle) {
    if (pg > 1) {
        return `<a href="?pg=` + (pg - 1) + navScrollTo + `"/>` + imgOrText(imageToggle, 1) + `</a>`;
    } else {
        return !imageToggle ? imgOrText(imageToggle, 1) : ``;
    }
}

function nextButton(imageToggle) {
    if (pg < maxpg) {
        return `<a href="?pg=` + (pg + 1) + navScrollTo + `"/>` + imgOrText(imageToggle, 2) + `</a>`;
    } else {
        return !imageToggle ? imgOrText(imageToggle, 2) : ``;
    }
}

function lastButton(imageToggle) {
    if (pg < maxpg) {
        return `<a href="?pg=` + maxpg + navScrollTo + `"/>` + imgOrText(imageToggle, 3) + `</a>`;
    } else {
        return !imageToggle ? imgOrText(imageToggle, 3) : ``;
    }
}

function divider(imageToggle) {
    return !imageToggle ? ` | ` : ``;
}

function writeNavLeft(imageToggle) {
    document.querySelectorAll(".writeNavLeft").forEach(function(element) {
        element.innerHTML = `<div class="comicNavSide">
            ${firstButton(imageToggle)}
            ${divider(imageToggle)}
            ${prevButton(imageToggle)}
        </div>`;
    });
}

function writeNavRight(imageToggle) {
    document.querySelectorAll(".writeNavRight").forEach(function(element) {
        element.innerHTML = `<div class="comicNavSide">
            ${nextButton(imageToggle)}
            ${divider(imageToggle)}
            ${lastButton(imageToggle)}
        </div>`;
    });
}
